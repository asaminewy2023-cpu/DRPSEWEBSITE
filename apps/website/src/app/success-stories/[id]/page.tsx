...
import { getSuccessStoryById } from "../../../lib/cms-data";
...
export default async function SuccessStoryDetail({ params }: { params: Promise<{ id: string }> }) {
  ...
  const item = await getSuccessStoryById(Number(id));
    ...
    <div className="bg-white">
      <section className="bg-gradient-to-br from-green-900 to-green-800 py-20">
        <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <Link href="/success-stories" className="inline-flex items-center gap-1 text-sm text-zinc-300 hover:text-white transition-colors mb-6">
            <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" strokeWidth="2" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" d="M10.5 19.5L3 12m0 0l7.5-7.5M3 12h18" />
            </svg>
            {t.successStories.backTo}
          </Link>
